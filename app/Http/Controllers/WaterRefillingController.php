...
class WaterRefillingController extends Controller
{
    protected $wr;
    //
    public function __construct(WaterRefilling $wr)
    {
        $this->wr = $wr;
    }
    public function index(Request $request)
    {
        ...
        DB::flushQueryLog();
        $data = WaterRefilling::where('longitude', '<>', '')
            ->where('latitude', '<>', '')
        ...
        // dd($dataG1);
        // dd($data->pluck('count'));
        $municipalities = WaterRefilling::where('municipality', '<>', 'MUNICIPALITY')
            ->where('municipality', '<>', '')
        ...
        $barangays = [];
        if ($mun) {
            $barangays = WaterRefilling::where('municipality', 'LIKE', '%' . $mun . '%')
                ->distinct('barangay')
        ...
        $puroks = [];
        if ($bar) {
            $puroks = WaterRefilling::where('barangay', 'LIKE', '%' . $bar . '%')
                ->distinct('purok_sitio')
                ->orderBy('purok_sitio', 'ASC')
                ->pluck('purok_sitio');
        }
        // dd(count($data));
        return inertia('WaterResource/Refilling/Index', [
            "p_data" => $data,
            // "g0" => $dataG0,
            // "g1" => $dataG1,
            // "g2" => $dataG2,
