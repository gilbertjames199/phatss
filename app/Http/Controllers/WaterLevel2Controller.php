...
use App\Models\WaterLevel2;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;

class WaterLevel2Controller extends Controller
{
    protected $w2;
    //
    public function __construct(WaterLevel2 $w2)
    {
        $this->w2 = $w2;
    }
    public function index(Request $request)
    {
        ...
        DB::flushQueryLog();
        $data = WaterLevel2::where('longitude', '<>', '')
            ->where('latitude', '<>', '')
        ...
        // dd($dataG1);
        // dd($data->pluck('count'));
        $municipalities = WaterLevel2::where('municipality', '<>', 'MUNICIPALITY')
            ->where('municipality', '<>', '')
        ...
        $barangays = [];
        if ($mun) {
            $barangays = WaterLevel2::where('municipality', 'LIKE', '%' . $mun . '%')
                ->distinct('barangay')
        ...
        $puroks = [];
        if ($bar) {
            $puroks = WaterLevel2::where('barangay', 'LIKE', '%' . $bar . '%')
                ->distinct('purok_sitio')
                ->orderBy('purok_sitio', 'ASC')
                ->pluck('purok_sitio');
        }
        // dd(count($data));
        return inertia('WaterResource/Level2/Index', [
            "p_data" => $data,
            // "g0" => $dataG0,
            // "g1" => $dataG1,
            // "g2" => $dataG2,
